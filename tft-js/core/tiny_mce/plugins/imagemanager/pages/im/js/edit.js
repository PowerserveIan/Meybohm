(function ($) {
	window.EditDialog = {
			...
			RPC.exec('im.getConfig', { path: t.imgPath }, function (data) {
				...
				if (!RPC.handleError({ message: 'Get config error', visual_path: t.args.visual_path, response: data })) {
					if (config['filesystem.clean_names'] == "true")
						$('#save_filename').val($.cleanName($('#save_filename').val().replace("{0}", "")));
		...
		crop: function () {
			...
			if (t.args.height != null) {
				f.crop_w.value = t.args.width;
				f.crop_h.value = t.args.height;
				f.crop_w.disabled = f.crop_h.disabled = f.crop_prop.disabled = f.crop_prop.checked = true;
				t.imageSelection.setRect(parseInt(f.crop_x.value), parseInt(f.crop_y.value), parseInt(f.crop_w.value), parseInt(f.crop_h.value));
			}
		},
			...
			RPC.exec('im.getMediaInfo', { path: o.path, url: o.url }, function (data) {
				...
				if (!RPC.handleError({ message: 'Generic error', response: data })) {
					...
					$('#editImage').load(function () {
						if (o.initial && t.args.height != null) {
							var f = document.forms[0];
							if ($("#editImage").height() >= $("#editImage").width()) {
								f.resize_w.value = t.args.width;
								f.resize_h.value = Math.round($("#editImage").height() * (parseInt(f.resize_w.value) / $("#editImage").width()));
							}
							else {
								f.resize_h.value = t.args.height;
								f.resize_w.value = Math.round($("#editImage").width() * (parseInt(f.resize_h.value) / $("#editImage").height()));
							}
							t.imageSelection.setRect(0, 0, parseInt(f.resize_w.value), parseInt(f.resize_h.value));
							t.execRPC('im.resizeImage', { path: t.imgPath, width: f.resize_w.value, height: f.resize_h.value, temp: true }, '{#error.resize_failed}');
							$(".selection-corner-tc, .selection-corner-cl, .selection-corner-cr, .selection-corner-bc, #resize_tools").remove();
							$("#resize").parent().remove();
							o.initial = null;
						}
						$.WindowManager.hideProgress();
					...
					$('#editImage').attr('src', '../../stream/default.aspx?cmd=im.streamFile&path=' + escape(res[0].path) + '&rnd=' + new Date().getTime());
