...
(function ($) {
	$.flexbox = function (div, o) {
		...
		if (o.initialText !== '')
			$input.val(o.initialText).removeClass('watermark');
		else
		...
		if (o.flexboxWidth != null)
			inputWidth = o.flexboxWidth - arrowWidth;
		...
		function displayItems(d, q) {
			...
			for (var i = 0; i < d[o.resultsProperty].length; i++) {
				...
				$row = (o.useTable ? $('<tr></tr>') : $('<div></div>'))
                    .attr('id', data[o.hiddenValue])
                    .attr('val', data[o.displayValue])
                    .addClass('row')
                    .html(result)
                    .appendTo((o.useTable ? $content.find("table") : $content));

				if (exactMatch || (++itemCount == 1 && o.selectFirstMatch) || selectedMatch || data[o.hiddenValue] == o.initialValue) {
			...
			$content
				.find(o.useTable ? 'tr.row' : 'div')
				.mouseover(function () {
					$content.find(o.useTable ? 'tr' : 'div').removeClass(o.selectClass);
		...
		function hideResults() {
			...
			$(".ffb .row").each(function () {
				if ($hdn.val() == $(this).attr("val"))
					$hdn.val($(this).attr("id"));
			});
		}
		...
		function getCurr() {
			...
			var $curr = $content.find((o.useTable ? 'tr' : 'div') + '.' + o.selectClass);
		...
		function nextPage() {
			...
			if ($curr && $curr.next().length > 0) {
				...
				$content.find((o.useTable ? 'tr' : 'div') + ':first-child').addClass(o.selectClass);
		}
		...
		function prevPage() {
			...
			if ($curr && $curr.prev().length > 0) {
				...
				$content.find((o.useTable ? 'tr' : 'div') + ':last-child').addClass(o.selectClass);
		}
	...
	$.fn.flexbox.defaults = {
		...
		initialText: '', // what should the value of the input field be when the form is loaded?
