...
namespace Classes.SiteLanguages
{
	public partial class Settings
	{
		...
		{
			get { return ConfigurationManager.AppSettings["SiteLanguages_connectionStringName"]; }
		}
		...
		public static int FrontEndPageSize
		{
			get
			{   
				if (SiteSettings.GetSettingKeyValuePair().ContainsKey("SiteLanguages_frontEndPageSize"))
					return Convert.ToInt32(SiteSettings.GetSettingKeyValuePair()["SiteLanguages_frontEndPageSize"]);
		...
		public static bool EnableCaching
		{
			get 
			{ 
				if (String.IsNullOrEmpty(ConfigurationManager.AppSettings["SiteLanguages_enableCaching"]))
					return Globals.Settings.EnableCaching;
				return Convert.ToBoolean(ConfigurationManager.AppSettings["SiteLanguages_enableCaching"]); 
			}
		}
		...
		public static int CacheDuration
		{
			get
			{   
				if (String.IsNullOrEmpty(ConfigurationManager.AppSettings["SiteLanguages_cacheDuration"]))
					return Globals.Settings.DefaultCacheDuration;
				int duration = Convert.ToInt32(ConfigurationManager.AppSettings["SiteLanguages_cacheDuration"]);
