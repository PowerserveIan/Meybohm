...
namespace Classes.ConfigurationSettings
{
	public partial class Settings
	{
		...
		{
			get { return ConfigurationManager.AppSettings["ConfigurationSettings_connectionStringName"]; }
		}
		...
		public static int FrontEndPageSize
		{
			get
			{   
				if (SiteSettings.GetSettingKeyValuePair().ContainsKey("ConfigurationSettings_frontEndPageSize"))
					return Convert.ToInt32(SiteSettings.GetSettingKeyValuePair()["ConfigurationSettings_frontEndPageSize"]);
		...
		public static bool EnableCaching
		{
			get 
			{ 
				if (String.IsNullOrEmpty(ConfigurationManager.AppSettings["ConfigurationSettings_enableCaching"]))
					return Globals.Settings.EnableCaching;
				return Convert.ToBoolean(ConfigurationManager.AppSettings["ConfigurationSettings_enableCaching"]); 
			}
		}
		...
		public static int CacheDuration
		{
			get
			{   
				if (String.IsNullOrEmpty(ConfigurationManager.AppSettings["ConfigurationSettings_cacheDuration"]))
					return Globals.Settings.DefaultCacheDuration;
				int duration = Convert.ToInt32(ConfigurationManager.AppSettings["ConfigurationSettings_cacheDuration"]);
