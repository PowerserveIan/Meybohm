...
namespace Classes.StateAndCountry
{
	public partial class Settings
	{
		...
		{
			get { return ConfigurationManager.AppSettings["StateAndCountry_connectionStringName"]; }
		}
		...
		public static int FrontEndPageSize
		{
			get
			{   
				if (SiteSettings.GetSettingKeyValuePair().ContainsKey("StateAndCountry_frontEndPageSize"))
					return Convert.ToInt32(SiteSettings.GetSettingKeyValuePair()["StateAndCountry_frontEndPageSize"]);
		...
		public static bool EnableCaching
		{
			get 
			{ 
				if (String.IsNullOrEmpty(ConfigurationManager.AppSettings["StateAndCountry_enableCaching"]))
					return Globals.Settings.EnableCaching;
				return Convert.ToBoolean(ConfigurationManager.AppSettings["StateAndCountry_enableCaching"]); 
			}
		}
		...
		public static int CacheDuration
		{
			get
			{   
				if (String.IsNullOrEmpty(ConfigurationManager.AppSettings["StateAndCountry_cacheDuration"]))
					return Globals.Settings.DefaultCacheDuration;
				int duration = Convert.ToInt32(ConfigurationManager.AppSettings["StateAndCountry_cacheDuration"]);
