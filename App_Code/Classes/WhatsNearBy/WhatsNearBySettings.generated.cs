...
namespace Classes.WhatsNearBy
{
	public partial class Settings
	{
		...
		{
			get { return ConfigurationManager.AppSettings["WhatsNearBy_connectionStringName"]; }
		}
		...
		public static int FrontEndPageSize
		{
			get
			{   
				if (SiteSettings.GetSettingKeyValuePair().ContainsKey("WhatsNearBy_frontEndPageSize"))
					return Convert.ToInt32(SiteSettings.GetSettingKeyValuePair()["WhatsNearBy_frontEndPageSize"]);
		...
		public static bool EnableCaching
		{
			get 
			{ 
				if (String.IsNullOrEmpty(ConfigurationManager.AppSettings["WhatsNearBy_enableCaching"]))
					return Globals.Settings.EnableCaching;
				return Convert.ToBoolean(ConfigurationManager.AppSettings["WhatsNearBy_enableCaching"]); 
			}
		}
		...
		public static int CacheDuration
		{
			get
			{   
				if (String.IsNullOrEmpty(ConfigurationManager.AppSettings["WhatsNearBy_cacheDuration"]))
					return Globals.Settings.DefaultCacheDuration;
				int duration = Convert.ToInt32(ConfigurationManager.AppSettings["WhatsNearBy_cacheDuration"]);
