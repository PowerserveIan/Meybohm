...
namespace Classes.ContentManager
{
	public partial class Settings
	{
		...
		{
			get { return ConfigurationManager.AppSettings["ContentManager_connectionStringName"]; }
		}
		...
		public static int FrontEndPageSize
		{
			get
			{   
				if (SiteSettings.GetSettingKeyValuePair().ContainsKey("ContentManager_frontEndPageSize"))
					return Convert.ToInt32(SiteSettings.GetSettingKeyValuePair()["ContentManager_frontEndPageSize"]);
		...
		public static bool EnableCaching
		{
			get 
			{ 
				if (String.IsNullOrEmpty(ConfigurationManager.AppSettings["ContentManager_enableCaching"]))
					return Globals.Settings.EnableCaching;
				return Convert.ToBoolean(ConfigurationManager.AppSettings["ContentManager_enableCaching"]); 
			}
		}
		...
		public static int CacheDuration
		{
			get
			{   
				if (String.IsNullOrEmpty(ConfigurationManager.AppSettings["ContentManager_cacheDuration"]))
					return Globals.Settings.DefaultCacheDuration;
				int duration = Convert.ToInt32(ConfigurationManager.AppSettings["ContentManager_cacheDuration"]);
