...
namespace BaseCode.HttpModules
{
	...
	public sealed class CompressionModule : IHttpModule
	{
		...
		private void context_PostReleaseRequestState(object sender, EventArgs e)
		{
			...
			{
				if (app.Context.Request.AppRelativeCurrentExecutionFilePath.ToLower().Contains("~/tft-js/core/tiny_mce/plugins/imagemanager") ||
					app.Context.Request.AppRelativeCurrentExecutionFilePath.ToLower().Contains("~/tft-js/core/tiny_mce/plugins/filemanager"))
					return;
		...
		{
			if (Globals.Settings.EnableHttpCompression)
			{
				context.PreRequestHandlerExecute += context_PostReleaseRequestState;
			}
		}
